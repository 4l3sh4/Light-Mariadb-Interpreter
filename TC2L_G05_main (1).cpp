...
int temp_int;
int *pointer4 = &temp_int;

string temp_string;
string *pointer5 = &temp_string;
...
// WAFIYYAH
string updateTable (string& command, ofstream& outputFile, string& tableName, vector<string> colNames, vector<string> colTypes){
    istringstream ss(command);
    string word, columns, inputTableName;
    bool foundUpdateTable = false;
    bool foundAttName = false;
    int swapFirst;
    int swapSecond;
    bool foundDataWhere = false;
    string setWhich;

    // Search "UPDATE TABLE" and extract table name
    while (ss >> word) {
        if (word == "UPDATE") {
            ss >> inputTableName; // get the table name
            foundUpdateTable = true;
            break;
        }
    }

    if (!foundUpdateTable) {
        cout << "Invalid UPDATE TABLE command!" << endl;
        return "";
    }
    ...
    while (ss >> word) {
        if (word == "SET") {
            ...
            cout << "Error: Missing SET command" << endl;
            return "";
        }
    }

    while (ss >> word){
        int equal_place = word.find('=');
        string updateWhich = word.substr(0, equal_place);
        for (int i=0; i < (*pointer1).size(); i++){
            if ((*pointer1)[i] == updateWhich){
                foundAttName = true;
                swapFirst = i;
                if((*pointer2)[i] == "INT"){
                    word = word.erase(0, equal_place + 2);
                    word.pop_back();

                    temp_int = stoi(word); // The updated value
                    cout << temp_int << endl << endl;
                }
                if((*pointer2)[i] == "TEXT"){
                    cout << word;
                    word = word.erase(0, equal_place + 2);
                    ...
                    temp_string = word; // The updated value
                    cout << temp_string << endl << endl;
                }
                break;
                }
            else{
        ...
        while (ss >> word) {
            if (word == "WHERE") {
                ...
                cout << "Error: Missing WHERE command" << endl;
                return "";
        }
    }
        while (ss >> word){
                int equal_place = word.find('=');
                setWhich = word.substr(0, equal_place);
                cout << setWhich << endl << endl;
        }

        for (int i=0; i < (*pointer1).size(); i++){
            if ((*pointer1)[i] == setWhich){
                foundAttName = true;
                swapSecond = i;
                /* if((*pointer2)[i] == "INT"){
                    int dataWhere;
        ...
        equal_place = word.find('=');
        word = word.erase(0, equal_place + 1);
        word.pop_back();
        /* word = word.erase(remove(word.begin(), word.end(), ";"), word.end()); */

        dataWhere = word; // Where we need to update

        cout << "> UPDATE " << inputTableName << " SET " << updateWhich << "=" << temp_string << " WHERE " << setWhich << "=" << dataWhere << ";";

       /* for (auto v: (*pointer3)) {
            for (auto i : v){
                if ((*pointer3)[i][v] == dataWhere){
                (*pointer3)[i][v] = temp;
                break;
                }
            }
                ...
                if (input[0] == "READ" && input[1] == "FROM" && input[2] == "FILE") {
                    ...
                    if (inputfile.is_open()) {
                        ...
                        while (getline(inputfile, words)) {
                            ...
                            else if (words.find("UPDATE") != string::npos){
                                ofstream outputFile;
                                updateTable(words, outputFile, tableName, *pointer1, *pointer2);
                                for (auto v: *pointer3) {
                                    for (auto i : v){
                                        cout << i << endl;
                                        }
